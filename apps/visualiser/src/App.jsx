...
function TagList({ items }) {
	if (!items?.length)
		return <span className='muted-copy'>None shown in current pin list.</span>;
	return (
...
function StageDiagram({ kind }) {
		...
		return (
			...
			>
				<rect
					...
					height='130'
					rx='30'
					className='tank-shape'
					style={{ '--tank-stroke': colorMap[kind] }}
				/>
				<rect
					x='42'
					y='98'
				...
				<text x='168' y='61' className='diagram-label'>
					{labels[0]}
				</text>
				<text x='168' y='100' className='diagram-label'>
					{labels[showFloats ? 3 : 1]}
				</text>
				...
				{showFloats ? (
					<>
						...
						<text x='168' y='36' className='diagram-label muted'>
							{labels[1]}
						</text>
						<text x='168' y='75' className='diagram-label muted'>
							{labels[2]}
						</text>
						<text x='168' y='115' className='diagram-label'>
							{labels[4]}
						</text>
					</>
				) : null}
	...
	if (kind === 'atu') {
		return (
			...
			>
				...
				<text x='220' y='74' className='diagram-label'>
					Blower outputs
				</text>
				<text x='220' y='102' className='diagram-label muted'>
					Pressure proof
				</text>
			</svg>
		);
	...
	if (kind === 'chlorinator') {
		return (
			<svg
				...
				viewBox='0 0 280 180'
				role='img'
				aria-label='chlorinator diagram'
			>
				...
				<text x='68' y='132' className='diagram-label muted'>
					Zone valves and pressure checks downstream
				</text>
			</svg>
		);
	}
	...
	return (
		...
		>
			...
			<text x='64' y='126' className='diagram-label dark'>
		...
		<article className='stage-card'>
			<div className='stage-card-head'>
				<div className='stage-card-title-row'>
					<h3>{stage.title}</h3>
					{stage.countChip ? (
						<span className='stage-count-chip'>{stage.countChip}</span>
					) : null}
				</div>
				<div>
					<p>{stage.summary}</p>
						...
						<div className='pinout-scroll'>
							<table className='pinout-table'>
								...
								<tbody>
									{group.rows.map(
										(
											[id, description, slot, channel, board, address],
											index,
										) => (
											<tr
												key={`${group.type}-${id}-${slot}-${channel}-${index}`}
											>
												<td>
													<span className='pinout-id'>{id}</span>
												</td>
												<td>{description}</td>
												<td>{slot}</td>
												<td>{channel}</td>
	...
	return (
		<div className='page'>
			<header className='page-header'>
				...
				<p>
					The best current description of the system flow and control logic,
					using the plan labels exactly as shown.
				</p>
			</header>
						...
						<article key={question.id} className='question-card'>
							<div className='question-head'>
								...
								<div className='question-copy'>
									<p>{question.prompt}</p>
									{question.note ? (
										<span className='question-note'>{question.note}</span>
									) : null}
								</div>
							</div>
						</article>
